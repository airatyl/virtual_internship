...
class TravelPremiumUnderwritingTest {
    ...
    void calculatePremiumForSelectedRisks() {
        ...
        Mockito.when(calculator.calculatePremium(request)).thenReturn(BigDecimal.ONE);
        Mockito.when(tripCalculator.calculatePremium(request)).thenReturn(BigDecimal.ONE);
        Mockito.when(request.getRisks()).thenReturn(List.of("Медицинские расходы","Отмена поездки"));
        TravelPremiumCalculationResult answers= underwriting.calculatePremiumForSelectedRisks(request);
